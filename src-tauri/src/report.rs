...
pub fn generate(
    ...
    let full = serde_json::json!({
        ...
        "tool": concat!("AD HyperOptimize v", env!("CARGO_PKG_VERSION")),
    ...
    let cpu = scan["cpu"]["Name"]
        .as_str()
        .or_else(|| scan["cpu"][0]["Name"].as_str())
        .unwrap_or("?");

    let hist_rows: String = history
        .as_array()
        .map(|h| {
            h.iter()
                .rev()
                ...
                .map(|e| {
                    format!(
                        ...
                        if e["reverted"].as_bool() == Some(true) {
                            "reverted"
                        } else {
                            "applied"
                        }
