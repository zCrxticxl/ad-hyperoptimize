...
static BLOAT: &[BloatEntry] = &[
    BloatEntry {
        name: "DiagTrack",
        reason: "Microsoft Telemetry (Connected User Experiences)",
    },
    BloatEntry {
        name: "dmwappushservice",
        reason: "WAP Push - Telemetry Routing",
    },
    BloatEntry {
        name: "WerSvc",
        reason: "Windows Error Reporting",
    },
    BloatEntry {
        name: "WSearch",
        reason: "Windows Search / Indexing (unnecessary if search unused)",
    },
    BloatEntry {
        name: "SysMain",
        reason: "Superfetch - can cause I/O spikes on SSD",
    },
    BloatEntry {
        name: "XblAuthManager",
        reason: "Xbox Live Auth - not needed without Xbox login",
    },
    BloatEntry {
        name: "XblGameSave",
        reason: "Xbox Live Game Save",
    },
    BloatEntry {
        name: "XboxNetApiSvc",
        reason: "Xbox Live Networking",
    },
    BloatEntry {
        name: "XboxGipSvc",
        reason: "Xbox Game Input Protocol",
    },
    BloatEntry {
        name: "MapsBroker",
        reason: "Offline Maps - automatic updates",
    },
    BloatEntry {
        name: "PcaSvc",
        reason: "Program Compatibility Assistant",
    },
    BloatEntry {
        name: "Fax",
        reason: "Legacy Fax Service",
    },
    BloatEntry {
        name: "stisvc",
        reason: "Windows Image Acquisition - only needed for scanners",
    },
    BloatEntry {
        name: "TapiSrv",
        reason: "Telephony API - Legacy",
    },
    BloatEntry {
        name: "lltdsvc",
        reason: "Link Layer Topology Discovery",
    },
    BloatEntry {
        name: "FDResPub",
        reason: "Function Discovery Resource Publication",
    },
    BloatEntry {
        name: "SSDPSRV",
        reason: "SSDP Discovery (UPnP)",
    },
    BloatEntry {
        name: "upnphost",
        reason: "UPnP Device Host",
    },
    BloatEntry {
        name: "RemoteRegistry",
        reason: "Remote Registry Access - security risk",
    },
    BloatEntry {
        name: "RetailDemo",
        reason: "Retail Demo Mode - not needed on a normal PC",
    },
    BloatEntry {
        name: "WMPNetworkSvc",
        reason: "Windows Media Player Network Sharing",
    },
    BloatEntry {
        name: "wisvc",
        reason: "Windows Insider Service",
    },
    BloatEntry {
        name: "wuauserv",
        reason: "Windows Update - only disable if self-managed",
    },
];
...
"#;
    ...
    let annotated: Vec<Value> = services
        .into_iter()
        .map(|mut s| {
    ...
    let bloat_count = annotated
        .iter()
        .filter(|s| s["isBloat"].as_bool().unwrap_or(false))
        .count();
...
pub fn set_startup(name: String, startup_type: String) -> Result<Value, String> {
    ...
    let script =
        format!("Set-Service -Name '{name}' -StartupType {startup_type} -ErrorAction Stop");
    ps::run(&script)?;
...
pub fn control(name: String, action: String) -> Result<Value, String> {
    ...
    let status_script =
        format!("(Get-Service -Name '{name}' -ErrorAction SilentlyContinue).Status.ToString()");
    let status = ps::run(&status_script)
        .unwrap_or_default()
        .trim()
        .to_string();
